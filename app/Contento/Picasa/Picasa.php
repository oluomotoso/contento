...
class Picasa extends Factory implements OAuth
{
    ...
    protected function doUpload($file, $token, $content_type)
    {
        ...
        $request = $this->createRequest($this->getAlbumUrl($albumId), 'POST')
            ->withHeader($this->getGeneralHeaders($token))
     ...
     */
    protected function doTransload($url, $token, $content_type)
    {
        ...
        try {
            $result = $this->doUpload($tempFile,$token,$content_type);
        } catch (Exception $e) {
            // ??
        }
    ...
    protected function createNewAlbum($title, $access = 'public', $description = '', $token)
    {
        $this->checkAndRefreshToken();
    ...
    private function getGeneralHeaders($token)
    {
        //$token = $this->getToken();
